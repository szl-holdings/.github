...
"""Final authority runner: cross-repo proof plus operator-PR isolation."""
...
OPERATOR_REPOSITORY = f"{reconciler.ORG}/{reconciler.CENTRAL_REPO}"
OPERATOR_BRANCH = "ops/estate-reconciliation-2026-09-04-v1"
...
_original_terminal_snapshot = reconciler.terminal_snapshot


def terminal_snapshot(
    api: Any, repositories: list[dict[str, Any]]
) -> dict[str, Any]:
    snapshot = _original_terminal_snapshot(api, repositories)
    open_prs = list(snapshot.get("open_pull_requests") or [])
    operator_prs = [
        row
        for row in open_prs
        if row.get("repository") == OPERATOR_REPOSITORY
        and row.get("head") == OPERATOR_BRANCH
    ]
    product_prs = [row for row in open_prs if row not in operator_prs]
    snapshot["operator_pull_requests"] = operator_prs
    snapshot["open_pull_requests"] = product_prs
    counts = dict(snapshot.get("counts") or {})
    counts["operator_pull_requests"] = len(operator_prs)
    counts["open_pull_requests"] = len(product_prs)
...
reconciler.terminal_snapshot = terminal_snapshot
