...
class PortfolioError(RuntimeError):
    """A malformed contract, unavailable authority, or provider drift."""
...
def safe(value: Any) -> str:
    """Bound diagnostics and redact credential-shaped material."""
...
def _require_text(row: Mapping[str, Any], field: str, name: str) -> None:
...
def validate_manifest(value: Mapping[str, Any]) -> None:
    ...
    if value.get("organization") != "szl-holdings":
        raise PortfolioError("organization must be szl-holdings")
    ...
    for index, row in enumerate(rows):
        ...
        if disposition == "restore":
            if targets:
                raise PortfolioError(
                    f"{name}: restore rows cannot delegate authority"
                )
            restores.add(name)
            ...
            raise PortfolioError(
                f"{name}: historical rows cannot declare a target"
            )
        ...
        for field in (
            ...
            _require_text(row, field, name)
        if not str(row["hugging_face_showcase"]).startswith("SZLHOLDINGS/"):
            raise PortfolioError(
                f"{name}: showcase must remain inside SZLHOLDINGS"
            )

    source = value.get("source_inventory")
    if (
        not isinstance(source, dict)
        or source.get("archived_public_repositories_observed") != 34
    ):
    ...
    if (
        wave.get("repositories") != sorted(restores)
    ...
    if restores != EXACT_RESTORES:
        raise PortfolioError(
            "restore set must be exactly "
            + ", ".join(sorted(EXACT_RESTORES))
        )
...
def build_plan(
    manifest: Mapping[str, Any], repositories: list[RepoState]
) -> dict[str, Any]:
    by_name = {repo.name: repo for repo in repositories}
    rows = {str(row["name"]): row for row in manifest["repositories"]}
    missing = sorted(set(rows) - set(by_name))
    if missing:
        raise PortfolioError(
            "manifest repositories missing: " + ", ".join(missing)
        )
    ...
    for name, row in sorted(rows.items()):
        ...
        if state.private:
            raise PortfolioError(
                f"{name}: classified public source became private"
            )
    ...
    if active_tombstones:
        raise PortfolioError(
            "consolidated or historical repositories unexpectedly active: "
            + ", ".join(active_tombstones)
        )
    if target_defects:
        raise PortfolioError(
...
class GitHubClient:
    def __init__(self, token: str):
        if not token:
            raise PortfolioError(
                "repository-administration credential unavailable"
            )
        self.token = token
    ...
    def repositories(self, organization: str) -> list[RepoState]:
        ...
        for page in range(1, 10):
            ...
            values = self.request(
                "GET", f"/orgs/{organization}/repos?{query}"
            )
    ...
    def repository(self, organization: str, name: str) -> RepoState:
        ...
        if not isinstance(value, dict):
            raise PortfolioError(
                f"{name}: repository response is not an object"
            )
    ...
    def require_admin(self, organization: str, names: list[str]) -> None:
        defects = [
            name
            for name in names
            if not self.repository(organization, name).admin
        ]
        if defects:
            raise PortfolioError(
                "credential lacks repository administration for: "
                + ", ".join(defects)
            )
    ...
    def unarchive(self, organization: str, name: str) -> RepoState:
        ...
        if (
            ...
            or before.disabled
            or before.fork
            or before.default_branch != "main"
        ):
            raise PortfolioError(
                f"{name}: precondition changed before unarchive"
            )

        response = self.request(
            "PATCH",
            f"/repos/{organization}/{name}",
            {"archived": False},
        )
        immediate = RepoState.from_api(
        ...
        for observed in (immediate, after):
            if observed.archived:
                raise PortfolioError(
                    f"{name}: provider readback remains archived"
                )
            if (
...
def execute(
    manifest: Mapping[str, Any], GitHub_client: GitHubClient, apply: bool
) -> dict[str, Any]:
    organization = str(manifest["organization"])
    plan = build_plan(
        manifest, GitHub_client.repositories(organization)
    )
    actions: list[dict[str, Any]] = []
    if apply and plan["restore"]:
        GitHub_client.require_admin(organization, plan["restore"])
    ...
    for name in plan["restore"]:
        if not apply:
            ...
            continue
        try:
            state = GitHub_client.unarchive(organization, name)
            actions.append(
                {
                    "repository": name,
    ...
    return {
        ...
        "final": final,
        "failures": failures,
        "unarchive_state_mutations_attempted": bool(
            apply and plan["restore"]
        ),
...
def write_report(path: Path, report: Mapping[str, Any]) -> None:
    ...
    if TOKEN_RE.search(text):
        raise PortfolioError(
            "credential-shaped material detected in report"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("governance/archive-portfolio-v2.json"),
    )
    parser.add_argument(
        "--mode", choices=("validate", "audit", "apply"), default="validate"
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("/tmp/archive-portfolio-v2.json"),
    )
    ...
    try:
        ...
        try:
            write_report(args.report, report)
        except PortfolioError as write_exc:
            print(
                "warning: secondary report write failed: " + safe(write_exc),
                file=sys.stderr,
            )
        print(
