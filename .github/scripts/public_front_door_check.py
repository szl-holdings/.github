...
class SurfaceParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "a" and values.get("href"):
            ...
            self.card_link_count += int("card" in classes)
            self.path_link_count += int("path" in classes)
        if tag == "h1":
            self.h1_count += 1
        if tag == "main":
            self.main_count += 1
...
def main() -> int:
    ...
    require(
        'src="assets/evidence-lattice-v2.webp"' in html,
        "static front door does not use the canonical WebP hero",
        failures,
    )
    require(
    ...
    require(
        ("assets/evidence-lattice-v2.webp", "") in parser.images,
