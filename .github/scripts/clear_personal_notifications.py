...
def clear_inbox(token: str) -> dict[str, Any]:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        # A small number of concurrently-created or API-lagged threads may survive
        # a bulk timestamp operation. Mark only those exact unread thread IDs.
    ...
    # Final bulk pass catches notifications created during the individual fallback.
    if remaining:
        mark_all_read(token)
        time.sleep(3)
        remaining = unread_thread_ids(token)
