...
@pytest.mark.parametrize("quoted_key", ['"sdk"', "'sdk'"])
def test_quoted_duplicate_front_matter_keys_fail_closed(
    tmp_path: Path,
    quoted_key: str,
) -> None:
    _fixture(tmp_path)
    ...
    manifest_path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(MODULE.ContractError, match="duplicate front-matter keys"):
        MODULE.validate(tmp_path, Path("docs/hf-universal-frontend-v1.json"))


def test_escaped_css_property_override_fails_closed(tmp_path: Path) -> None:
    _fixture(tmp_path)
    css = tmp_path / "szl-universal-frontend.css"
    css.write_text(
        CSS + "\nhtml { overflow\\-x: auto !important; }\n",
...
@pytest.mark.parametrize(
    "conditional_rule",
    [
        "@supports (display: block) { html { overflow-x: auto !important; } }",
        "@media (min-width: 1px) { html { overflow-x: auto !important; } }",
        "@container (min-width: 1px) { html { overflow-x: auto !important; } }",
    ],
)
def test_unmodeled_conditional_css_fails_closed(
    tmp_path: Path,
    ...
    with pytest.raises(
        ...
        match="unsupported conditional or at-rule block",
    ):
        MODULE.validate(tmp_path, Path("docs/hf-universal-frontend-v1.json"))


def test_streamlit_binding_rejects_positional_keyword_collision(
    tmp_path: Path,
) -> None:
    _fixture(tmp_path, "streamlit")
    app = tmp_path / "app.py"
    app.write_text(
        app.read_text(encoding="utf-8").replace(
            "st.markdown(f'<style>{_SZL_UNIVERSAL_CSS}</style>', "
            "unsafe_allow_html=True)",
            "st.markdown(f'<style>{_SZL_UNIVERSAL_CSS}</style>', True, "
            "unsafe_allow_html=True)",
        ),
        encoding="utf-8",
    )
    ...
    with pytest.raises(
        MODULE.ContractError,
        match="Streamlit universal CSS binding is absent",
    ):
        MODULE.validate(tmp_path, Path("docs/hf-universal-frontend-v1.json"))
...
def test_python_binding_after_known_raise_handler_fails_closed(
    tmp_path: Path,
    framework: str,
    handler: str,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "raise RuntimeError from (1 / 0)",
    ],
)
def test_python_binding_after_raise_evaluation_failure_is_rejected(
    tmp_path: Path,
    framework: str,
    raise_statement: str,
) -> None:
...
def test_python_binding_after_shadowed_exception_name_is_rejected(
    tmp_path: Path,
    framework: str,
    shadowing: str,
) -> None:
