...
**Guardian for the SOW.** Holistic Guardian watches model actions. ServicePath watches quote-to-GL. Kantata, Certinia, and ConnectWise keep the record. Nobody issues inspectable receipts on SOW → staff → exception → invoice. That membrane sits between CRM and PSA. Overlay. Do not replace.

## Recapture (authenticated + public APIs)
...
Kantata, Certinia, ConnectWise PSA, Autotask (Kaseya), HaloPSA, NetSuite OpenAir, Rocketlane, Credo AI, Holistic AI (Guardian Agents), ServicePath (CRM-GL missing mile). Overlay only.
