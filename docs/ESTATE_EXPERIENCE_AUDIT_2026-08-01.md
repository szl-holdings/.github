...
- Replaced the single-point-of-failure SVG hero with a 116 KB WebP that is
  published by the same exact-source manifest as the Hugging Face front door.
- Added absolute, durable organization-profile asset routing.
- Rebuilt the first viewport as a layered composition with a readable fallback
  background and hard-contrast actions even if the image fails.
- Added Open Graph metadata tied to the same managed asset.
- Reduced the GitHub organization profile to one mission sentence, three
  audience paths, two products, one system map, and explicit evidence language.
...
- Updated the publication contract and local verifier to validate WebP format,
  file size, references, and manifest inclusion.
