...
This successor is scoped only to the GitHub and Hugging Face organization front
doors. Until protected merge and exact live readback complete, it is proposed
source—not a deployed upgrade. It does **not** claim that every repository,
model card, dataset card, or Space has been restyled or operationally verified.
...
3. four mission paths — command, intelligence, models and data, evidence;
...
teal/indigo/gold accents, hard-contrast actions, and four mission paths with
separate investor, builder, and verification routes.
...
| --- | --- | --- | --- |
| `.github` | Active public | Organization front door and governance | **P0 — successor in review; protect the canonical visual, truth boundaries, and four mission paths** |
...
| --- | --- |
| `README` | Successor proposes a canonical thumbnail and local reproduction path; verify the exact served revision after merge |
...
## Successor scope and validation contract

- Uses a 116 KB WebP as the canonical visual source with a
...
- Publishes that one profile asset to the Hugging Face destination while
  preserving all established SVG compatibility URLs under `prune: true`.
- Adds Open Graph metadata tied to the same managed asset.
- Replaces mobile-hostile profile tables with stacked paths while retaining
  artifact classes, product boundaries, current-state sources, and evidence
  language.
- Adds a Hugging Face thumbnail and a local reproduction command.
- Extends the publication contract and local verifier to validate WebP format,
  dimensions, file size, metadata limits, truth markers, responsive and
  accessibility contracts, structural workflow triggers, and exact
  source-to-destination manifest bindings.
